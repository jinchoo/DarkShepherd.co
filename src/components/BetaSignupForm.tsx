...
import { isValidHttpUrl, normalizeStoreUrl } from "@/lib/storeUrl";
...
export function BetaSignupForm() {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...
      shopifyUrl: normalizeStoreUrl(String(data.get("shopifyUrl") ?? "")),
    ...
    if (!isValidHttpUrl(payload.shopifyUrl)) {
      setErrorMessage("Please enter a valid Shopify store URL, like yourstore.myshopify.com");
      setStatus("error");
      return;
    }
    ...
    return (
      <div className="rounded-2xl border border-amber-300/25 bg-white/[0.03] px-6 py-8 text-center">
        ...
        <p className="mt-2 text-sm leading-relaxed text-slate-400">
          We&apos;ll review your beta application and be in touch.
        </p>
      </div>
    );
  }
